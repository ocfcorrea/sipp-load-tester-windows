...
Estilo Slate / Dark Navy Suave com paleta moderna e limpa.
"""
...
class TabStrategy(ctk.CTkScrollableFrame):
    ...
    def _build_ui(self):
        ...
        # -------------------------------------------------------------
        cap_card = ctk.CTkFrame(self, corner_radius=8, fg_color="#272a37", border_width=1, border_color="#383c4e")
        ...
        lbl_cap_title.pack(anchor="w", padx=16, pady=(12, 6))

        grid_cap = ctk.CTkFrame(cap_card, fg_color="transparent")
        grid_cap.pack(fill="x", padx=16, pady=4)
        grid_cap.grid_columnconfigure((0, 1, 2, 3), weight=1)

        # Simultâneas (-l)
        f_simult = ctk.CTkFrame(grid_cap, fg_color="transparent")
        f_simult.grid(row=0, column=0, sticky="ew", padx=4, pady=3)
        ctk.CTkLabel(f_simult, text="Simultâneas (-l)*", font=ctk.CTkFont(size=11, weight="bold"), text_color="#94a3b8").pack(anchor="w")
        self.entry_simultaneas = ctk.CTkEntry(f_simult, placeholder_text="100", fg_color="#1e202b", border_color="#383c4e", text_color="#f1f5f9", height=30)
        self.entry_simultaneas.pack(fill="x", pady=(2, 0))

        # Total de chamadas (-m)
        f_total = ctk.CTkFrame(grid_cap, fg_color="transparent")
        f_total.grid(row=0, column=1, sticky="ew", padx=4, pady=3)
        ctk.CTkLabel(f_total, text="Total (-m) [0=Ilimitado]*", font=ctk.CTkFont(size=11, weight="bold"), text_color="#94a3b8").pack(anchor="w")
        self.entry_total = ctk.CTkEntry(f_total, placeholder_text="0", fg_color="#1e202b", border_color="#383c4e", text_color="#f1f5f9", height=30)
        self.entry_total.pack(fill="x", pady=(2, 0))

        # Duração Mínima (ms)
        f_dur_min = ctk.CTkFrame(grid_cap, fg_color="transparent")
        f_dur_min.grid(row=0, column=2, sticky="ew", padx=4, pady=3)
        ctk.CTkLabel(f_dur_min, text="Duração Mínima (ms)*", font=ctk.CTkFont(size=11, weight="bold"), text_color="#94a3b8").pack(anchor="w")
        self.entry_dur_min = ctk.CTkEntry(f_dur_min, placeholder_text="10000", fg_color="#1e202b", border_color="#383c4e", text_color="#f1f5f9", height=30)
        self.entry_dur_min.pack(fill="x", pady=(2, 0))

        # Duração Máxima (ms)
        f_dur_max = ctk.CTkFrame(grid_cap, fg_color="transparent")
        f_dur_max.grid(row=0, column=3, sticky="ew", padx=4, pady=3)
        ctk.CTkLabel(f_dur_max, text="Duração Máxima (ms)*", font=ctk.CTkFont(size=11, weight="bold"), text_color="#94a3b8").pack(anchor="w")
        self.entry_dur_max = ctk.CTkEntry(f_dur_max, placeholder_text="60000", fg_color="#1e202b", border_color="#383c4e", text_color="#f1f5f9", height=30)
        ...
        self.chk_dur_fixa.pack(anchor="w", padx=20, pady=(4, 12))

        # -------------------------------------------------------------
        # CARD 2: MODO DE DISCAGEM & SIMULAÇÃO HUMANA RANDÔMICA
        # -------------------------------------------------------------
        mode_card = ctk.CTkFrame(self, corner_radius=8, fg_color="#272a37", border_width=1, border_color="#383c4e")
        ...
        lbl_mode_title.pack(anchor="w", padx=16, pady=(12, 6))
        ...
        f_rate.grid(row=0, column=0, sticky="ew", padx=4)
        ctk.CTkLabel(f_rate, text="Novas Chamadas por Período (-r)*", font=ctk.CTkFont(size=11, weight="bold"), text_color="#94a3b8").pack(anchor="w")
        self.entry_rate = ctk.CTkEntry(f_rate, placeholder_text="50", fg_color="#1e202b", border_color="#383c4e", text_color="#f1f5f9", height=30)
        self.entry_rate.pack(fill="x", pady=(2, 0))

        f_period = ctk.CTkFrame(self.frame_rate_mode, fg_color="transparent")
        f_period.grid(row=0, column=1, sticky="ew", padx=4)
        ctk.CTkLabel(f_period, text="Período da Taxa em ms (-rp)*", font=ctk.CTkFont(size=11, weight="bold"), text_color="#94a3b8").pack(anchor="w")
        self.entry_rate_period = ctk.CTkEntry(f_period, placeholder_text="1000", fg_color="#1e202b", border_color="#383c4e", text_color="#f1f5f9", height=30)
        ...
        ctk.CTkLabel(f_h_min, text="Intervalo Mín (ms)", font=ctk.CTkFont(size=11, weight="bold"), text_color="#94a3b8").pack(anchor="w")
        self.entry_human_min = ctk.CTkEntry(f_h_min, placeholder_text="200", fg_color="#1e202b", border_color="#383c4e", text_color="#f1f5f9", height=30)
        ...
        ctk.CTkLabel(f_h_max, text="Intervalo Máx (ms)", font=ctk.CTkFont(size=11, weight="bold"), text_color="#94a3b8").pack(anchor="w")
        self.entry_human_max = ctk.CTkEntry(f_h_max, placeholder_text="1500", fg_color="#1e202b", border_color="#383c4e", text_color="#f1f5f9", height=30)
        ...
        ctk.CTkLabel(f_h_burst, text="Chance de Pico (%)", font=ctk.CTkFont(size=11, weight="bold"), text_color="#94a3b8").pack(anchor="w")
        self.entry_human_burst = ctk.CTkEntry(f_h_burst, placeholder_text="15", fg_color="#1e202b", border_color="#383c4e", text_color="#f1f5f9", height=30)
        ...
        ctk.CTkLabel(f_h_token, text="Prefixo de Token", font=ctk.CTkFont(size=11, weight="bold"), text_color="#94a3b8").pack(anchor="w")
        self.entry_human_token = ctk.CTkEntry(f_h_token, placeholder_text="AGENT_", fg_color="#1e202b", border_color="#383c4e", text_color="#f1f5f9", height=30)
        self.entry_human_token.pack(fill="x", pady=(2, 0))

        # -------------------------------------------------------------
        # CARD 3: TABELA DE DESTINOS COM PESOS E PRIORIDADES
        # -------------------------------------------------------------
        dest_card = ctk.CTkFrame(self, corner_radius=8, fg_color="#272a37", border_width=1, border_color="#383c4e")
    ...
    def _on_save_button(self):
        if self.save_to_config():
            messagebox.showinfo("Estratégia Salva", "A estratégia de carga, capacidade e tabela de destinos foram salvas com sucesso no arquivo config.json!")
    ...
    def _distribute_equal_weights(self):
        for row in self.dest_table.rows:
            if row["chk_var"].get() and row["entry_num"].get().strip():
                row["slider"].set(10)
